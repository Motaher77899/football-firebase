...
class _HomeScreenState extends State<HomeScreen> {
  ...
  MatchModel? _tournamentMatchToMatchModel(
    ...
    try {
      ...
      return MatchModel(
        ...
        tournament: data['tournamentId'] ?? '', // ✅ Tournament indicator
        venue: data['venue'] ?? '', teamAId: '', teamBId: '', adminFullName: '', createdAt:DateTime.now(),
      );
    } catch (e) {
  ...
  // ✅ NEW: Open tournament match details
  Future<void> _openTournamentMatchDetails(
      MatchModel match,
      TeamProvider teamProvider,
      ) async {
    try {
      debugPrint('🔍 Opening tournament match: ${match.id}');

      // Fetch complete tournament match data
      ...
      if (!doc.exists) {
        debugPrint('❌ Tournament match not found: ${match.id}');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('ম্যাচ খুঁজে পাওয়া যায়নি')),
          );
        }
        return;
      }

      // Convert to TournamentMatch model
      final tournamentMatch = TournamentMatch.fromMap(
        doc.data() as Map<String, dynamic>,
        doc.id,
      );

      debugPrint('✅ Tournament match loaded with ${tournamentMatch.timeline.length} timeline events');

      // Navigate to TournamentMatchDetailsScreen
      if (mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
      ...
      debugPrint('❌ Error opening tournament match details: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  // ✅ UPDATED: Smart navigation based on match type
    ...
    return GestureDetector(
      onTap: () {
        ...
        if (match.tournament != null && match.tournament!.isNotEmpty) {
          // Tournament match - open TournamentMatchDetailsScreen
          debugPrint('🏆 Opening tournament match: ${match.tournament}');
          _openTournamentMatchDetails(match, teamProvider);
        } else {
          // Regular match - open MatchDetailsScreen
          debugPrint('⚽ Opening regular match');
          Navigator.push(
            context,
            MaterialPageRoute(
