...
import 'package:provider/provider.dart';
import 'package:intl/intl.dart';
// 💡 আবশ্যক সংশোধন: Timestamp ব্যবহারের জন্য এই প্যাকেজটি ইম্পোর্ট করতে হবে।
import 'package:cloud_firestore/cloud_firestore.dart';
import '../providers/player_provider.dart';
...
class _MyPlayerProfileScreenState extends State<MyPlayerProfileScreen>
    ...
    return Scaffold(
      ...
      body: Consumer<PlayerProvider>(
        builder: (context, playerProvider, child) {
          ...
          if (player == null) {
            // ⏳ লোডিং স্টেট দেখাবে যখন প্লেয়ার ডেটা লোড হচ্ছে, অথবা যদি কোনো এরর থাকে।
            ...
            // যদি প্রোফাইল না থাকে (যেমন, নতুন ইউজার)
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Text(
                    'আপনার কোনো প্লেয়ার প্রোফাইল নেই।',
        ...
        if (snapshot.hasError) {
          return Center(
            child: Padding(
              ...
              child: Text(
                'ম্যাচ লোড করতে ব্যর্থ: ${snapshot.error}',
                textAlign: TextAlign.center,
                style: const TextStyle(color: Colors.red),
              ),
            ),
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const Text(
                  'আপনার এলাকার কোনো ম্যাচ খেলা হয়নি',
                  style: TextStyle(
                    color: Colors.white70,
                    fontSize: 16,
                  ),
  ...
  Widget _buildMatchCard(Map<String, dynamic> match) {
    ...
    final scoreA = match['scoreA'] ?? 0;
    final scoreB = match['scoreB'] ?? 0;
    // 💡 সংশোধন ঠিক করা হয়েছে: Timestamp ব্যবহার করা হয়েছে
    final date = (match['date'] as Timestamp?)?.toDate() ?? DateTime.now();
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              success
                                  ? 'পজিশন আপডেট সফল হয়েছে'
                                  : playerProvider.errorMessage ?? 'আপডেট ব্যর্থ হয়েছে',
                            ),
                            backgroundColor:
                            success ? Colors.green : Colors.red,
                          ),
