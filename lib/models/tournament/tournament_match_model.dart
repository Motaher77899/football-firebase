...
// ============================================================================
// TOURNAMENT MATCH MODEL (Null-Safe & Error-Free)
// ============================================================================
class TournamentMatch {
  final String id;
  final String tournamentId;
  ...
  // ✅ Null-Safe fromMap Factory
  factory TournamentMatch.fromMap(Map<String, dynamic> map, String id) {
    List<TimelineEvent> timelineEvents = [];

    // ✅ Safe Timeline Parsing
    try {
      if (map['timeline'] != null && map['timeline'] is List) {
        final timelineList = map['timeline'] as List;

        timelineEvents = timelineList.map((e) {
          try {
            if (e is Map<String, dynamic>) {
              return TimelineEvent.fromMap(e);
            } else if (e is Map) {
              // Convert to Map<String, dynamic> if different Map type
              final eventMap = Map<String, dynamic>.from(e);
              return TimelineEvent.fromMap(eventMap);
            }
            return null;
          } catch (err) {
            debugPrint('❌ Error parsing timeline event: $err');
            return null;
          }
        }).whereType<TimelineEvent>().toList();

        debugPrint('✅ Parsed ${timelineEvents.length} timeline events');
      }
    } catch (e) {
  ...
  // ✅ Helper: Parse DateTime safely (handles null Timestamp)
  static DateTime _parseDateTime(dynamic value) {
    if (value == null) {
      debugPrint('⚠️ DateTime is null, using current time');
      return DateTime.now();
    }
    ...
    if (value is String) {
      try {
        ...
        debugPrint('❌ Error parsing DateTime string: $e');
        return DateTime.now();
      }
    }

    debugPrint('⚠️ Unknown DateTime type: ${value.runtimeType}');
    return DateTime.now();
  }
...
// ============================================================================
// TIMELINE EVENT MODEL (Null-Safe)
// ============================================================================
class TimelineEvent {
  final int minute;
  final String playerId;
  ...
  // ✅ Display Minute with apostrophe
  String get displayMinute => "$minute'";
  ...
  TimelineEvent copyWith({
    ...
    return TimelineEvent(
      ...
      playerOutName: playerOutName ?? this.playerOutName,
      playerInId: playerInId ?? this.playerInId,
      playerInName: playerInName ?? this.playerInName,
    );
  }
}